...
class PembayaranReqTable extends BaseWidget
{
        ...
        return $table
            ->query(
                DebtRequestPayment::with(['debt_request.debtor', 'debt_request.creditor'])->whereHas('debt_request', function (Builder $q) {
                    $q->where('creditor_user_id', auth()->id());
    ...
    protected function hitungSisaSaldo($set, $moneyPlacingId, $record)
    {
        if (!$moneyPlacingId) {
            $set('sisa_saldo', 'Rp. 0');
            return;
        ...
        // dd($biayaAdmin) ;
        //penambahan money placing dari sini
        $sisa = $moneyPlacing->amount + $record->debt_request->amount;
